...
function updateSiteStats() {
  ...
  for (var i = 0; i < cookieList.length; i++) {
    if (cookieList[i].isProtected) nProtected++;
  }
  $("#statTotal b").text(cookieList.length);
  $("#statProtected b").text(nProtected);
  if (nProtected > 0) {
    $("#statProtected").show();
  } else {
    $("#statProtected").hide();
...
function initFirstRunHint() {
  if (localStorage.getItem("hint_firstRunClosed") !== null) return;
  $("#firstRunBanner").show();
  $("#firstRunClose").click(function () {
    localStorage.setItem("hint_firstRunClosed", "1");
    $("#firstRunBanner").fadeOut();
  });
}
...
function doSearch(isSeparateWindow) {
  ...
  try {
    $("#siteHostname").text(getHost(url));
  } catch (e) {
    $("#siteHostname").text(url);
  }
...
function submit(currentTabID) {
  if (newCookie) submitNew(currentTabID);
  else if (pasteCookie) importCookies();
  else submitAll(currentTabID);
}
...
function importCookies(text) {
  ...
  text = text || $(".value", "#pasteCookie").val();
...
function setEvents() {
  ...
  $("#copyButton")
    .unbind()
    .click(function () {
      var cookiesWithLocalDomain = cookieList.map(function (cookie) {
        var c = Object.assign({}, cookie);
        c.domain = "127.0.0.1";
        return c;
      });
      var localCookie = cookiesToString.get(cookiesWithLocalDomain);
      importCookies(localCookie);
      copyToClipboard(localCookie);
      ...
      $(this).animate(
        { backgroundColor: "rgba(46, 158, 68, 0.3)" },
        300,
        function () {
          $(this).animate({ backgroundColor: "rgba(255, 255, 255, 0.55)" }, 500);
        },
      );
    });
